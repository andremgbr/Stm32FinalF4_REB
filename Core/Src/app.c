...
uint8_t monitor_read_can(void) {
	uint8_t status = ECU_SUCCESS;
	while (status == ECU_SUCCESS) {

		can_frame frame = { .can_id = 29, .can_dlc = 8, .data = { 0xFF,
...
uint8_t handle_frame_can(can_frame *frame) {
	...
	if (frame->can_id == TCU_REB_ID) {
		if (handle_tcu_can(frame->data) == ECU_FAIL) {
	...
	if ((frame->can_id == AUX_COM_ID) && (frame->data[0] == AUX_COM_SIG)) {
		...
		if (can_send_vcan0(&response) == ECU_FAIL) {
...
uint8_t start_reb(void) {
	...
	if (reb_can_send_ipc(IPC_REB_START) == ECU_FAIL) {
...
uint8_t countdown_reb(void) {
	while (1) {
		...
		if (read_pin_status(&reb_countdown_active, REB_COUNTDOWN_PIN) == ECU_FAIL) {
		...
		while (reb_countdown_active != 0U) {
			...
			if (elapsed_time >= (double) REB_TIMEOUT) {
				if (reb_can_send_ecu(ECU_REB_START) == ECU_FAIL) {
					show_error("reb_can_send_ecu ERROR\n");
				}

				if (set_pin_status(S_OFF, REB_COUNTDOWN_PIN) == ECU_FAIL) {
					show_error("set_pin_status ERROR\n");
				}
			}

			// Verify the status of the pin again
			if (read_pin_status(&reb_countdown_active, REB_COUNTDOWN_PIN)
					== ECU_FAIL) {
				show_error("read_pin_status ERROR\n");
			}
		}
