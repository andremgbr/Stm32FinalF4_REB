...
uint8_t handle_tcu_can(const unsigned char *data) {
	...
	if (signalREB == (uint8_t) 0x01U) {
		...
		if (set_pin_status(S_ON, REB_COUNTDOWN_PIN) == ECU_FAIL) {
			show_error("set_pin_status ERROR\n");
		}

		if (start_reb() == ECU_FAIL) {
			REPORT_ERROR("tcu_can.start_reb ECU_FAIL\n",
...
uint8_t reb_can_send_ecu(uint8_t status) {
	uint8_t statusReturn = ECU_SUCCESS;
	...
	if (can_send_vcan0(&frame) == ECU_FAIL) {
		statusReturn = ECU_FAIL;
	}
...
uint8_t reb_can_send_ipc(uint8_t status) {
	...
	can_frame frame = { .can_id = REB_IPC_ID, .can_dlc = 8, .data = { 0 } };
